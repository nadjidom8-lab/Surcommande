...
function App() {
  ...
  const handleDownload = useCallback(async (o) => {
    ...
    if (isMobile) {
      // On mobile: open image in new tab — user can long-press → Save Image
      ...
      if (w) {
        w.document.write(`
          ...
          <title>طلبية ${o.orderNumber}</title>
          ...
          p{color:#fff;font-family:Arial;font-size:14px;text-align:center;padding:0 16px}</style>
          </head><body>
          <p>اضغط مطولاً على الصورة ثم اختر "حفظ الصورة"</p>
          <img src="${dataUrl}" alt="طلبية ${o.orderNumber}"/>
          </body></html>
        `);
        w.document.close();
        setDlMsg("✅ افتح التاب الجديد واحفظ الصورة");
      } else {
        // Popup blocked — fallback: show image in an overlay inside the app
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'Amiri',serif", direction:"rtl", color:C.text }}>
      ...
      {/* Image Preview Overlay (mobile fallback when popup blocked) */}
      {imgPreviewUrl && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.92)", zIndex:3000, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:16, gap:16 }}
          onClick={() => setImgPreviewUrl(null)}>
          <div style={{ color:"#fff", fontSize:14, fontFamily:"'Amiri',serif", textAlign:"center", background:"rgba(255,255,255,.1)", borderRadius:12, padding:"10px 20px" }}>
            اضغط مطولاً على الصورة ثم اختر <strong>حفظ الصورة</strong>
          </div>
          <img src={imgPreviewUrl} alt="طلبية" style={{ maxWidth:"100%", maxHeight:"75vh", borderRadius:12, boxShadow:"0 8px 40px rgba(0,0,0,.5)" }}
            onClick={(e) => e.stopPropagation()} />
          <button onClick={() => setImgPreviewUrl(null)}
            style={{ background:"rgba(255,255,255,.15)", border:"1px solid rgba(255,255,255,.3)", color:"#fff", borderRadius:10, padding:"10px 28px", fontSize:15, fontFamily:"'Amiri',serif", cursor:"pointer" }}>
            إغلاق
          </button>
        </div>
      )}
